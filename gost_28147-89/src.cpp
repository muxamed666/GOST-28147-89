...
int main(void) {
    uint8_t output[BUFF_SIZE];
    uint8_t key256b[32] = { 116, 104, 105, 115, 95, 105, 115, 95, 97, 95, 112, 97, 115, 119, 95, 102, 111, 114, 95, 071, 79, 83, 84, 95, 50, 56, 49, 52, 55, 95, 56, 57 };

    printf("===================\nGOST 28147-89 encryption/decryption program\n===================\n\n");

    //get key
    char key_filename[32];
    printf("Enter key file name: ");
    scanf("%s", &key_filename);
    FILE* keyfile_dsc = fopen(key_filename, "rb");
    if (!keyfile_dsc)
    {
    ...
    fread(key256b,sizeof(uint8_t), 32, keyfile_dsc);

    //get open text
    uint8_t buffer[BUFF_SIZE];
    size_t position = 0;

    char opentext_filename[32];
    printf("Enter input file name: ");
    scanf("%s", &opentext_filename);
    FILE* opentext_dsc = fopen(opentext_filename, "rb");
    if (!opentext_dsc)
    {
        printf("ERROR! FILE %s NOT FOUND!\n", opentext_filename);
        getchar();
    ...
    position = fread(buffer, sizeof(uint8_t), BUFF_SIZE, opentext_dsc);
    buffer[position] = '\0';
    ...
    if (ans == 'E')
    {
        position = GOST_28147(output, 'E', key256b, buffer, position);
        printf("Encrypted bytes:\n");
        print_array(output, position);
        //printf("%s\n", encrypted);
        putchar('\n');
    }
    ...
    if (ans == 'D')
    {
        position = GOST_28147(output, 'D', key256b, buffer, position);
    ...
    printf("Enter output file name (will be overwritten if exists): ");
    scanf("%s", &out_filename);
    FILE* out_dsc = fopen(out_filename, "w+b");
    if (!out_dsc)
    {
    ...
    fwrite(output, sizeof(uint8_t), position, out_dsc);
    fclose(out_dsc);
